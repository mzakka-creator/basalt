...
import BlogEvents from '@/app/components/BlogEvents/BlogEvents';
...
import type { BlogCategory, BlogEventRecord, BlogPostRecord } from '@/lib/cms/types';
import {
  IconArrowRight,
  IconClock,
  IconFileText,
...
export default function BlogPageClient({ posts, events }: BlogPageClientProps) {
  ...
  const postsInGrid = posts.filter(
    (p) => !p.featured && (activeFilter === 'all' || p.category === activeFilter)
  );

  useScrollAnimation([activeFilter, postsInGrid.length]);

  const getTitle = (post: BlogPostRecord) => (locale === 'ar' ? post.titleAr : post.titleEn);
  const getExcerpt = (post: BlogPostRecord) => (locale === 'ar' ? post.excerptAr : post.excerptEn);
